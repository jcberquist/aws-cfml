component {
	...
	public any function init( required any api, string region = 'us-east-1', string apiVersion = '20120810' ) {
	...
	*/
	public any function listTables(
		...
		return apiCall( 'ListTables', payload );
	}
	...
	*/
	public any function createTable(
		...
		var payload = buildPayload( arguments );
		return apiCall( 'CreateTable', payload );
	}
	...
	*/
	public any function describeTable(
		...
		var payload = buildPayload( arguments );
		var apiResponse = apiCall( 'DescribeTable', payload );
		return apiResponse;
	}
	...
	*/
	public any function updateTable(
		...
		var payload = buildPayload( arguments );
		return apiCall( 'UpdateTable', payload );
	}
	...
	*/
	public any function deleteTable(
		...
		var payload = buildPayload( arguments );
		var apiResponse = apiCall( 'DeleteTable', payload );
		return apiResponse;
	}
	...
	*/
	public any function putItem(
		...
		var payload = buildPayload( arguments );
		var apiResponse = apiCall( 'PutItem', payload );
		return apiResponse;
	}
	...
	*/
	public any function batchGetItem(
		...
		var apiResponse = apiCall( 'BatchGetItem', payload );
	...
	*/
	public any function batchWriteItem(
		...
		var apiResponse = apiCall( 'BatchWriteItem', payload );
		return apiResponse;
	}
	...
	*/
	public any function query(
		...
		var payload = buildPayload( arguments );
		var apiResponse = apiCall( 'Query', payload );
	...
	*/
	public any function scan(
		...
		var apiResponse = apiCall( 'Scan', payload );
		if ( apiResponse.data.keyExists( 'Items' ) ) {
			apiResponse.data.Items = apiResponse.data.Items.map( decodeValues );
		}
		if ( apiResponse.data.keyExists( 'LastEvaluatedKey' ) ) {
	...
	public struct function encodeValues(
		...
		return structMap( data, function( key, value ) {
			...
			return encodeAttributeValue( value, typeDefinitions.keyExists( key ) ? typeDefinitions[ key ] : determineValueType( value ) );
		} );
	}
	...
	private any function apiCall(
		...
		var apiResponse = api.call( variables.service, variables.host, variables.region, 'POST', '/', { }, headers, payloadString );
	...
	private struct function encodeAttributeValue(
		...
		switch( typeDefinition ) {
			...
			case 'L':
				return { 'L': data.map( function( item ) { return encodeAttributeValue( item, determineValueType( item ) ); } ) };
			case 'M':
				return { 'M': structMap( data, function( key, value ) { return encodeAttributeValue( data[ key ], determineValueType( data[ key ] ) ); } ) };
		}
