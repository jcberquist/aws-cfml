component {
    ...
    public any function getSystemSetting(
        required string key,
        any defaultValue
    ) {
    ...
    public any function parseXmlDocument(
        required string xmlDocument
    ) {
        return parseXmlNode( xmlParse( xmlDocument ).xmlRoot );
    }

    public any function parseXmlElement(
        required string xmlDocument,
    ...
    public any function parseXmlNode(
        ...
        for ( var thisChild in xmlNode.xmlChildren ) {
            ...
            if ( !result.keyExists( thisChild.XmlName ) ) {
                ...
                if ( !isArray( result[ thisChild.XmlName ] ) )
                    result[ thisChild.XmlName ] = [ result[ thisChild.XmlName ] ];
    ...
    public string function parseKey(
    ...
    public string function parseQueryParams(
    ...
    public string function encodeUrl(
        required string str,
        boolean encodeSlash = true
    ) {
        var result = replaceList( encodeForURL( arguments.str ), '%2D,%2E,%5F,%7E,+,*', '-,.,_,~,%20,%2A' );
        if ( !encodeSlash ) result = replace( result, '%2F', '/', 'all' );
        return result;
    }

    public string function iso8601(
        date dateToFormat = now()
    ) {
        return dateTimeFormat( dateToFormat, 'yyyymmdd', 'UTC' ) & 'T' & dateTimeFormat( dateToFormat, 'HHnnss', 'UTC' ) & 'Z';
    }

    public string function iso8601Full(
        date dateToFormat = now()
    ) {
        return dateTimeFormat( dateToFormat, 'yyyy-mm-dd', 'UTC' ) & 'T' & dateTimeFormat(
            dateToFormat,
            'HH:nn:ss',
            'UTC'
        ) & '.000Z';
